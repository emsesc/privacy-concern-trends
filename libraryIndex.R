...
library_data <- read.csv("./data/library-survey-2019.csv")
...
library_index_2019 <- read.csv("./outputs/clean_pen_index.csv") %>%
  select(code, county_state) %>%
  distinct() %>%
  left_join(library_data, by = "county_state") %>%
  group_by(code) %>%
  summarise(avg_index_2019 = mean(Library_Usage_Index, na.rm = TRUE), 
            num_lib_2019 = n(),
            avg_revenue_2019 = mean(Library_Revenue, na.rm = TRUE),
            avg_expenses_2019 = mean(Library_Expenses, na.rm = TRUE)) %>%
  ungroup() %>%
  mutate(std_avg_index_2019 = scale(avg_index_2019))

########################
full_library_index <- full_join(library_index_2019, library_index_2021, by = c("code"))
