...
```{r}
test2 <- google_trends_analysis %>% ungroup() %>% select(Trends_DMA) %>% distinct()
```
...
library <- read.csv("./outputs/library.csv")
library_index <- read.csv("./outputs/library_index.csv")
book_bans_serach <- read.csv("./outputs/book-bans-search.csv")
...
hits_combined_df <- full_join(libraries_near_me, library, by = c("X", "geo", "Month", "Year", "DMA", "STATE", "count")) %>%
  ...
  mutate(avg_hits_next_month = rowMeans(select(., starts_with("avg_hits_next_month")), na.rm = TRUE),
         avg_hits_prev_month = rowMeans(select(., starts_with("avg_hits_prev_month")), na.rm = TRUE))
...
# Fit OLS model
ols_model <- lm(std_avg_index ~ num_lib + count + offset(log(tot_pop)) + avg_hits_next_month, data = test3)
