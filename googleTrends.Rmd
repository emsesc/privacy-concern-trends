...
for (i in 1:3) {
  ...
  results_list[[i]] <- trend_data$interest_over_time
}
...
hit_change <- joined_df %>%
 ...
 select(geo, hits, Year, Month, avg_hits_prev_month, avg_hits_next_month)
...
hits_combined_df <- full_join(avg_hits_prev_month_1_avg, avg_hits_next_month_1_avg, by = c("geo", "Month", "Year"), suffix = c("_prev_month", "_next_month"))
```
